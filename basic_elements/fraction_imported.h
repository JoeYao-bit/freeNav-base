...
struct Fraction {
    ...
    inline bool isDivideEqualTo(int dvd) const {
        return d == dvd;
    }

    inline int round(bool half_to_ceil) const {
        auto to_float = toFloat();
        if(half_to_ceil) {
            return isDivideEqualTo(2) ? ceil() : static_cast<int>(std::round(to_float));
        } else {
            return isDivideEqualTo(2) ? floor() : static_cast<int>(std::round(to_float));
        }
    }

    inline Fraction toAbs() const {
        Fraction retv;
        retv.n = abs(n);
        retv.d = abs(d);
        return retv;
...
typedef std::vector<FractionPair> FractionPairs;
