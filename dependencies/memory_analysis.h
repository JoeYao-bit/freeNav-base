...
public:
    MemoryRecorder(int time_interval_ms) : time_interval_(time_interval_ms) {
        std::thread back_thread = std::thread([&] {
            while (1) {
                ...
                //std::cout << "max memory usage " << getMaximalMemoryUsage() << std::endl;
                std::this_thread::sleep_for(std::chrono::milliseconds(time_interval_));
            }
        });
        back_thread.detach();
    }
    ...
    std::vector<float> getAllUsedMemory() const {
        return used_memory_;
    }
...
private:
    const int time_interval_; // in mile seconds
    std::vector<float> used_memory_; // in MB
    std::mutex mutex_;

};

#endif //FREENAV_MEMORY_ANALYSIS_H
