...
freeNav::Pointi<3> Viewer3D::target_;

freeNav::Paths<2> Viewer3D::mapf_paths_;

int Viewer3D::current_top_level = 0;

int Viewer3D::current_path_id = 0;

int Viewer3D::max_time_index_ = 0;
...
void Viewer3D::DrawLine(const Pointi<3> &p1, const Pointi<3> &p2, float r, float g, float b) {
//    glLineWidth(4);
//    glBegin(GL_LINES);
//    glColor3f(r,g,b);
...
void Viewer3D::DrawLine(int x1, int y1, int z1, int x2, int y2, int z2, float r, float g, float b) {
    glLineWidth(4);
    glBegin(GL_LINES);
    glColor3f(r,g,b);
    glVertex3f(x1, y1, z1);
...
void Viewer3D::DrawVoxels(const Pointis<3>& pts, const cv::Vec3b& color) {
    for(const auto& pt : pts) {
        DrawVoxel(pt, color);
    }
}

void Viewer3D::DrawBlock(const Pointi<3>& p1, const Pointi<3>& p2, float r, float g, float b) {
...
void Viewer3D::DrawVoxels(const fr::Pointis<3>& voxels, fr::DimensionLength* dim) {
...
void Viewer3D::DrawBound(double min_x, double max_x, double min_y, double max_y, double min_z, double max_z) {
...
void Viewer3D::DrawVoxel(double x, double y, double z, float halfsize, double r, double g, double b, bool filled) {
    if(filled) {
...
void Viewer3D::DrawArrow(double x1, double y1, double x2, double y2, double z, double head_width, double line_width) {
    // the center pole
